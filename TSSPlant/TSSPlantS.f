...
c   KAUST 
c
c   Plant Promoter Prediction Program  (with Summary-statistics)
c
c   Input File: Sequence(s) in FASTA format  
c
c   RUN: TSSPlantS -i:<Input sequence(s) file> 
c             [-s: <annotated start position of gene/mRNA or CDS>] 
c             [-p:<print (y or Y) or not print (n or N) Query sequence(s)] 
...
c             [-b: <Left and right boundaries of region to select a single TSS; default: 300]  
c
c   Default value for <parS>: Length of Query SEQ 
c   Default value for <parP>: n (not print query SEQs)
c   Deault output file: TSSPlant.res 
...
c ___________________________________________________
        ...
        character*800 path 
         ...
         character*1 seq(1000),seqt(100000) 
         character*1 seqc(1000),seqtc(100000) 	 
  	 character*300 class(2) 
  	 character*300 pr(6),prr(6) 
  	 character*3 pp(6)	 
	 byte seqb(1000),seqtb(100000),cont(100000)
	 integer tata(100000),tatax(100000),ptype 
	 integer rex,tss(2,100000),ntss(2),kcls(2)
	 integer prt,h,tatapos(100000),tataposx 	
	 real frg(4),scortss(2,100000),scor(16),scorx
	 integer tss1(100000),tatapos1(100000),tss2(100000) 
	 real scortss1(100000),scortata1(100000),scortss2(100000) 	 
	 real pi,scortata(100000),scortatax,c(2),skx(11,2) 
	 ...
	 integer postata(2),ltata(2),nst(100000)
	 integer posinr(2,2),linr(2,2),nst2(100000)
...
c  ___________________________________________________
 ...
 4	format(i2) 
 5	format(f12.4)  
 6	format('Program TSSPlantS',/,'Search for RNA II promoters (TSSs)',/,
...
c ______ Get parameters ______ 
 ...
 1045			continue
	...
	do i=1,300
		if(pr(2)(i:i).ne.' ')goto 5987
	enddo
 5987	k1=i
 	k2=len1(pr(2))		
	mtss=intr(pr(2),k1,k2)
	if(mtss.lt.0)mtss=0
		
 3989 	prt=0
 	if((pr(3)(1:1).eq.'y').or.(pr(3)(1:1).eq.'Y'))prt=1

 	ptype=1
 	if((pr(5)(1:1).eq.'n').or.(pr(5)(1:1).eq.'N'))ptype=0

 	outfile=emp
	if(pr(4).eq.emp)then
		outfile='TSSPlant.res'
	else
		outfile=pr(4)
	endif 
	...
	do i=1,300
		if(pr(6)(i:i).ne.' ')goto 7987
	enddo
 7987	k1=i
 	k2=len1(pr(6))		
	inter=intr(pr(6),k1,k2)
...
c ___________________________________________________________________
	...
	print*,' '	    
	print*,'<parB>  Left and Right boundaries of an ',
